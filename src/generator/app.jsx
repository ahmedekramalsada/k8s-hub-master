...
function AIView({ currentType, currentYaml, theme, ai.enabled }) {
  ...
  const [model, setModel] = useState(() => localStorage.getItem("openrouter_model") || "google/gemma-3-27b-it:free");
...
Current resource type: ${currentType}
Current YAML:
\`\`\`yaml
${currentYaml}
\`\`\`

Help with: YAML review, security audits, best practices, troubleshooting (CrashLoopBackOff, OOMKilled, ImagePullBackOff, etc.), Kubernetes concepts.
Be concise and practical. Use code blocks for YAML examples. If asked in Arabic, respond in Arabic.`;
    ...
    try {
      ...
      const reply = data.content || "Sorry, no response received.";
      setMessages(m => [...m, { role: "assistant", content: reply }]);
    } catch (e) {
      setMessages(m => [...m, { role: "assistant", content: `❌ Error: ${e.message}\n\nCheck your API key and model name in settings ⚙️` }]);
    }
    setLoading(false);
  };
  ...
  const quickPrompts = [
    ...
    "What are K8s best practices for production?",
    "How do I set up HTTPS with cert-manager on k3s?",
  ];
  ...
  const renderMessage = (content) => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith("```")) {
        ...
        return <pre key={i} style={{ background: theme.yamlBg, border: `1px solid ${theme.border}`, borderRadius: 8, padding: "12px 16px", margin: "8px 0", fontSize: 11.5, color: "#a0f0c0", overflow: "auto", fontFamily: "'JetBrains Mono', monospace" }}>{code}</pre>;
      }
    ...
    return (
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 40, background: theme.bgCard }}>
        <div style={{ maxWidth: 400, textAlign: "center", animation: "fadeUp 0.3s ease" }}>
          ...
          <h2 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, color: theme.accent, marginBottom: 12 }}>AI Not Configured</h2>
          <p style={{ color: theme.textMuted, fontSize: 13, lineHeight: 1.6, marginBottom: 24 }}>
            To use the AI assistant, please configure the <code style={{ color: theme.accent, background: `${theme.accent}15`, padding: "2px 6px", borderRadius: 4 }}>OPENROUTER_API_KEY</code> on the server's environment variables.
          </p>
          <div style={{ background: theme.bgInput, border: `1px solid ${theme.border}`, borderRadius: 10, padding: 16, textAlign: "left" }}>
            <div style={{ color: theme.text, fontSize: 11, fontWeight: 700, marginBottom: 8, textTransform: "uppercase" }}>Quick Setup Guide:</div>
            <ol style={{ color: theme.textDim, fontSize: 11.5, paddingLeft: 18, margin: 0, lineHeight: 1.8 }}>
              <li>Get a key from <a href="https://openrouter.ai" target="_blank" rel="noopener noreferrer" style={{ color: theme.accent }}>openrouter.ai</a></li>
              <li>Add it to your <code>.env</code> file or cloud environment</li>
  ...
  return (
    <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
        {/* Header */}
        <div style={{ padding: "12px 20px", borderBottom: `1px solid ${theme.border}`, display: "flex", alignItems: "center", gap: 10, background: theme.bgCard }}>
          <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#4ade80", animation: "pulse 2s infinite" }} />
          <span style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, color: "#818cf8", fontSize: 15 }}>🤖 AI DevOps Assistant</span>
          <span style={{ color: theme.textDim, fontSize: 11 }}>via OpenRouter</span>
          <button onClick={() => setMessages([{ role: "assistant", content: "Chat cleared. How can I help?" }])} style={{ marginLeft: "auto", background: "transparent", border: `1px solid ${theme.border}`, borderRadius: 6, color: theme.textMuted, cursor: "pointer", fontSize: 11, padding: "4px 10px", fontFamily: "'JetBrains Mono', monospace" }}>Clear</button>
        </div>
          ...
          <div style={{ maxWidth: 820, margin: "0 auto", display: "flex", flexDirection: "column", gap: 14 }}>
            {messages.map((msg, i) => (
              <div key={i} style={{ display: "flex", justifyContent: msg.role === "user" ? "flex-end" : "flex-start" }}>
                <div style={{ maxWidth: "80%", padding: "12px 16px", borderRadius: msg.role === "user" ? "14px 14px 2px 14px" : "14px 14px 14px 2px", background: msg.role === "user" ? theme.accentSoft : theme.bgCard, border: `1px solid ${msg.role === "user" ? theme.accent + "40" : theme.border}`, color: theme.text, fontSize: 13, lineHeight: 1.7 }}>
            ...
            {loading && (
              <div style={{ display: "flex" }}>
                <div style={{ background: theme.bgCard, border: `1px solid ${theme.border}`, borderRadius: "14px 14px 14px 2px", padding: "14px 18px", display: "flex", gap: 5 }}>
                  {[0, 1, 2].map(i => <div key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: "#6366f1", animation: `pulse 1s ${i * 0.2}s infinite` }} />)}
                </div>
              </div>
            )}
          </div>
        ...
        {/* Quick Prompts */}
        <div style={{ padding: "8px 20px", borderTop: `1px solid ${theme.border}`, display: "flex", gap: 6, flexWrap: "wrap" }}>
          {quickPrompts.map(q => (
            <button key={q} onClick={() => setInput(q)}
              style={{ background: theme.bgCard, border: `1px solid ${theme.border}`, borderRadius: 20, color: theme.textMuted, cursor: "pointer", fontSize: 10.5, padding: "4px 12px", fontFamily: "'JetBrains Mono', monospace", transition: "all 0.15s" }}
        ...
        <div style={{ padding: "12px 20px", borderTop: `1px solid ${theme.border}`, display: "flex", gap: 8 }}>
          <textarea value={input} onChange={e => setInput(e.target.value)}
            ...
            placeholder="Ask about your YAML, K8s concepts, troubleshooting... (Enter to send)"
            style={{ flex: 1, background: theme.bgInput, border: `1px solid ${theme.border}`, borderRadius: 8, color: theme.text, fontFamily: "'JetBrains Mono', monospace", fontSize: 12.5, padding: "10px 14px", outline: "none", resize: "none", height: 52 }} />
          <button onClick={send} disabled={loading || !input.trim()}
            style={{ background: loading || !input.trim() ? theme.border : "linear-gradient(135deg, #4f46e5, #7c3aed)", border: "none", borderRadius: 8, color: "white", cursor: loading || !input.trim() ? "not-allowed" : "pointer", fontSize: 18, padding: "0 16px" }}>
            {loading ? "⏳" : "➤"}
          </button>
        </div>
      </div>
